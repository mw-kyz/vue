...
(function (global, factory) {
   ...
   * 尝试为值创建观察者实例，如果观察成功，则返回新的观察者，或者返回现有的观察器（如果值已经有一个观察器）。
   */
  ...
  // 返回_createElement方法，主要是为_createElement规范化参数
  ...
  function initMixin (Vue) {
    ...
    Vue.prototype._init = function (options) {
      ...
      /* istanbul ignore else */
      {
        // 使用Proxy方法代理vm对象，然后再重新赋值给vm._renderProxy 
        initProxy(vm);
      }
      // expose real self
      vm._self = vm;
      ...
      // 初始化渲染
      initRender(vm);
      // 触发beforeCreate钩子函数，执行钩子回调，当用户注册了this.$on('hook:beforeCreate', function() {})时，会调用emit发送beforeCreate事件
      callHook(vm, 'beforeCreate');
      // 在data和props属性之前注入inject，也就是初始化inject
      initInjections(vm); // resolve injections before data/props
      // 初始化data、props、methods、computed、watch
      initState(vm);
      // 在data和props之后注入provide，也就是初始化provide，provide是和inject搭配使用的，provide提供数据，inject接收数据
      initProvide(vm); // resolve provide after data/props
      // 触发created 钩子函数，执行钩子回调，，当用户注册了this.$on('hook:created', function() {})时，会调用emit发送created事件
      callHook(vm, 'created');
